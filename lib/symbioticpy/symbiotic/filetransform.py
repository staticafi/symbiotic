...
class NondetSimplify(FileTransform):
    ...
    def __init__(self):
        from re import compile
        self._re1 = compile(
            '^(\s*)if\s*\(\s*__VERIFIER_nondet_(int|uint)\(\s*\)\s*\)(.*)$')
        self._re2 = compile(
            '^(\s*)while\s*\(\s*__VERIFIER_nondet_(int|uint)\(\s*\)\s*\)(.*)$')
        ...
        for l in infile:
            ...
            if res:
                ...
                line = '{0}{1} (__VERIFIER_nondet__Bool()){2}\n'.format(
                    res.group(1), keyword, res.group(3))
                if version_info < (3, 0):
                    line = line.decode('utf-8')
            else:
                line = l
                ...
                if res:
                    ...
                    if version_info < (3, 0):
                        outfile.write('{0}volatile _Bool inf_true{1} = 1; while(inf_true{1}){2}\n'.format(
                            res.group(1), lines, res.group(2)).decode('utf-8'))
                    else:
                        outfile.write('{0}volatile _Bool inf_true{1} = 1; while(inf_true{1}){2}\n'.format(
                            res.group(1), lines, res.group(2)))
                    found = True
                    break
